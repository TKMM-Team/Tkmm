    ...
    private async Task Create(string output, bool cleanOutput)
    {
        try {
            ...
            if (string.IsNullOrEmpty(Mod.Name)) {
                Mod.Name = Path.GetFileName((string?)SourceFolder);
            }

            if (string.IsNullOrEmpty(ExportPath)) {
                ExportPath = Path.Combine(Path.GetDirectoryName((string?)SourceFolder) ?? string.Empty,
                    $"{Path.GetFileName((string?)SourceFolder)}.tkcl");
            }
